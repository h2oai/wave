...
def next_time_series_value(column, point):
    return (donut_prices[column][point]).item(), (donut_prices[column][point]).item()


def next_data_point():
    i = 0
    while True:
        yield (i, i % donut_prices.index.size)
        i += 1
...
next_data_point = next_data_point()
cards = []
for index, column in enumerate(columns):
    val, pc = next_time_series_value(column, index)
    stat_card = page.add(f'example{index}', ui.small_series_stat_card(
        box=f'1 {index + 1} 1 1',
        title=column,
