...
async def serve(q: Q):
    if q.args.train:
        ...
        model = build_model(train_file_path='wine_train.csv', target_column='target',
                                     model_type=ModelType.H2O3, _h2o3_max_runtime_secs=5, _h2o3_nfolds=0
        )
        
        # save model
        q.client.path_model = save_model(model=model, output_dir_path='./mymodelpath')
        ...
        # load model
        model = load_model(file_path=q.client.path_model)

        # predict on test data
        preds = model.predict(file_path='wine_test.csv')
