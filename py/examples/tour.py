...
class Example:
    ...
    async def start(self):
        ...
        env = os.environ.copy()
        env.update({'SYSTEMROOT': os.environ['SYSTEMROOT']} if sys.platform.lower().startswith('win') else {})
        if self.is_app:
            env.update(dict(H2O_WAVE_EXTERNAL_ADDRESS=f'http://{_app_host}:{_app_port}'))
