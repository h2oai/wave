...
func Run(conf ServerConf) {
	...
	broker := newBroker(site, conf.Editable, conf.NoStore, conf.NoLog, conf.NoAppDropIfUnresponsive)
	go broker.run()
	...
	handle("_s/", newSocketServer(broker, auth, conf.Editable, conf.BaseURL, conf.ForwardedHeaders)) // XXX terminate sockets when logged out
