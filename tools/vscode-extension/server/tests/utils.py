...
class BaseTestCase(unittest.TestCase):
    ...
    def get_completions(self, fake_content: str, inline=True, doc_uri: Optional[str] = None) -> List[str]:
        fake_document = Document('fake_uri', fake_content)
        self.server.workspace.get_document = Mock(return_value=fake_document)
        completion_params = FakeCompletionParams(0, len(fake_content), doc_uri) if inline else FakeCompletionParams(1, 2, doc_uri)
